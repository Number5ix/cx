...
// WASM NetSelectSet: same shape as unix_net_select.c's -- fd_set is a bitmask indexed by fd, so
// the set tracks every added handle itself (a plain growable array) and iterates that against
// FD_ISSET on the ready working sets. POSIX signals a failed connect through writability +
// SO_ERROR rather than a separate except set, so there is only a read/write pair here, not
// read/write/except. Unlike the Unix backend this is the *only* queue backend WASM has -- there is
// no epoll/kqueue/IOCP equivalent in Emscripten's syscall emulation to prefer over it.
...
void nselAdd(_Inout_ NetSelectSet* set, NetSockHandle h, bool read, bool write)
{
    ...
    // out of bounds rather than merely failing to track a socket. This is a hard cap: the socket
    // is silently left unwatched this pass rather than corrupting the set. Since select is the
    // only backend WASM has (no epoll/kqueue fallback target to fall back *from* the way Linux/
    // FreeBSD do), a socket hitting this cap here has no better backend to land on.
