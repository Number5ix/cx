...
_Use_decl_annotations_
void prqInitDynamic(PrQueue* prq, uint32 minsz, uint32 targetsz, uint32 maxsz, PrqGrowth growth,
                    PrqGrowth shrink)
{
    if (growth == 0)
        growth = PRQ_Grow_100;
...
fullretry:
    ...
    if (didreserve) {
        ...
        while ((reserved & RESERVED_MASK) > 0) {
            ...
            if (atomicCompareExchange(uint32,
                                      ...
                                      &seg->reserved,
                                      &reserved,
                                      (reserved & RESERVED_RETIRING) |
                                          ((reserved & RESERVED_MASK) - 1),
                                      Release,
                                      Acquire))
                break;
    ...
    if (full) {
        if (dynamic && seg->size < prq->maxsz) {
            ...
            if (prqGrow(prq, seg, prqGrowSize(prq->growth, seg->size), &gcoll)) {
                ...
                goto fullretry;   // will follow nextseg into the new segment the other thread
                                  // allocated
            }
        }
    ...
    if (dynamic) {
        ...
        for (;;) {
            ...
            // if the segment is being retired, or if we somehow hit 2 billion writers, need to
            // retry instead
    ...
    for (slot = head + count; !success && slot < head + count + seg->size; slot++) {
        ...
        while (oldptr == NULL) {
            if (atomicCompareExchange(ptr,
                ...
                // for perf reasons, we care that this happened on the suboptimal path of having to
                // scan every slot
                if (appearedfull)
...
void* prqPop(PrQueue* prq)
{
    ...
    bool empty           = false;
    bool havenext = false;   // if this segment has been replaced by growing, we always want to try
                             // to assist

    uint32 count = atomicLoad(uint32, &seg->count, Acquire);
    uint32 head  = atomicLoad(uint32,
                             &seg->head,
                             Relaxed);   // acquire/release on count maintains head ordering,
...
bool prqCollect(PrQueue* prq)
{
    ...
    bool canshrink = (now - chgtime > timeMS(prq->shrinkms)) ||
        (now < chgtime);   // handle rollover
    ...
    if (nextseg) {
        ...
        if (!(reserved & RESERVED_RETIRING)) {
            ...
            // CAS is not needed on current because nothing else is allowed to change this pointer
            // except GC, and we have the mutex
