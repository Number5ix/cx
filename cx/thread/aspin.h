...
_meta_inline void aspinBegin(_Inout_ AdaptiveSpin* aspin, _Out_ AdaptiveSpinState* ass,
                             int64 timeout)
{
    ass->now       = clockTimer();
    ass->start     = ass->now;
...
_meta_inline void aspinAdapt(_Inout_ AdaptiveSpin* aspin, _Inout_ AdaptiveSpinState* ass)
{
    ...
    if (ass->curtarget != ASPIN_NOSPIN) {
        if (ass->spincount > 0) {
            ...
            atomicFetchAdd(int32,
                           &aspin->spintarget,
                           clamp(ass->curtarget - ass->spincount, -realtarget, realtarget) / 8 + 1,
                           Relaxed);
...
_meta_inline void
aspinHandleContention(_Inout_opt_ AdaptiveSpin* aspin, _Inout_ AdaptiveSpinState* ass)
{
