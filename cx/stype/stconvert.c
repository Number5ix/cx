...
#define stConvertBoolNum(type)                                                       \
    case stTypeId(type):                                                             \
        dest->st_##type = src.st_bool ? (stTypeDef(type))(1) : (stTypeDef(type))(0); \
        return true
_Success_(return) _Check_return_ bool
...
#define stConvertSIntInput(type)    \
    case stTypeId(type):            \
        v.s = (int64)src.st_##type; \
        break
#define stConvertUIntInput(type)     \
    case stTypeId(type):             \
        v.u = (uint64)src.st_##type; \
        break

#define stConvertSIntOutput(type, vmin, vmax)                                        \
    case stTypeId(type):                                                             \
...
#define stConvertUIntOutput(type, vmax)         \
    case stTypeId(type):                        \
        if (!norange && v.u > vmax)             \
            return false;                       \
...
_Success_(return) _Check_return_ bool
stConvert_int(stype destst, _stCopyDest_Anno_(destst) stgeneric* dest, stype srcst,
              _In_ stgeneric src, uint32 flags)
{
    ...
    case stTypeId(float32):
        ...
        if (lossless && ((!srcunsigned && v.s < -16777216) || v.u > 16777216))
            return false;
        dest->st_float32 = srcunsigned ? (float32)v.u : (float32)v.s;
        return true;
    case stTypeId(float64):
        // check limits of what can be losslessly represented as a float
        if (lossless && ((!srcunsigned && v.s < -9007199254740992LL) || v.u > 9007199254740992ULL))
            return false;
...
_Success_(return) _Check_return_ bool
stConvert_float32(stype destst, _stCopyDest_Anno_(destst) stgeneric* dest, stype srcst,
                  _In_ stgeneric src, uint32 flags)
{
    bool norange = flags & ST_Overflow;
...
_Success_(return) _Check_return_ bool
stConvert_float64(stype destst, _stCopyDest_Anno_(destst) stgeneric* dest, stype srcst,
                  _In_ stgeneric src, uint32 flags)
{
    bool norange  = flags & ST_Overflow;
    bool lossless = flags & ST_Lossless;
...
_Success_(return) _Check_return_ bool
stConvert_ptr(stype destst, _stCopyDest_Anno_(destst) stgeneric* dest, stype srcst,
              _In_ stgeneric src, uint32 flags)
{
