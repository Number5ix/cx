...
//
// Returns:
//   File handle on success, NULL on failure
//   Common failures: file doesn't exist (without FS_Create), no permission,
//                    path is a directory, disk full (with FS_Create)
//
// Example:
...
//
// Returns:
//   true if the read operation succeeded (even if 0 bytes read due to EOF)
//   false if an I/O error occurred (bytesread will be set to 0)
//
// Example:
//   uint8 buffer[1024];
//   size_t n;
...
//
// Returns:
//   true if the write succeeded (check byteswritten for actual amount)
//   false if an I/O error occurred
//   Common failures: disk full, quota exceeded, permission denied
//
// Example:
//   uint8 data[] = { 1, 2, 3, 4 };
//   size_t written;
...
//
// Returns:
//   Current file position in bytes (0 = start of file)
//   -1 on error
...
//
// Returns:
//   New file position in bytes from start of file
//   -1 on error (invalid position, unseekable file)
//
// Examples:
