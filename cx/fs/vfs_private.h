...
_Ret_valid_ _When_(!writelockheld, _Requires_shared_lock_held_(vfs->vfslock)) VFSDir*
_vfsGetDir(_Inout_ VFS* vfs, _In_opt_ strref path, bool isfile, bool cache, bool writelockheld);
// gets a file from VFS cache if it exists
// must be called with vfslock read (or write) lock held!
_Ret_valid_ _When_(!writelockheld, _Requires_shared_lock_held_(vfs->vfslock)) VFSCacheEnt*
_vfsGetFile(_Inout_ VFS* vfs, _In_opt_ strref path, bool writelockheld);
...
_Requires_shared_lock_held_(vfs->vfslock) int _vfsFindCIHelper(_Inout_ VFS* vfs, _In_ VFSDir* vdir,
                                                               _Inout_ string* out,
                                                               _In_ sa_string components,
                                                               _Inout_ VFSMount* mount,
