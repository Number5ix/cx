...
/// @defgroup fs_vfs Virtual Filesystem
/// @ingroup fs
/// @{
///
...
///
/// @param pvfs Pointer to VFS handle to destroy
...
/// @return true if successfully unmounted, false on error
bool vfsUnmount(_Inout_ VFS* vfs, _In_opt_ strref path);

bool _vfsMountProvider(_Inout_ VFS* vfs, _Inout_ ObjInst* provider, _In_opt_ strref path,
                       flags_t flags);
...
#define vfsMountProvider(vfs, provider, path, ...) \
...
///
/// @param vfs VFS instance
/// @param out Receives the current directory path
...
///
/// @param vfs VFS instance
/// @param out Receives the absolute path
...
/// @return true if path exists (as any type), false otherwise
...
/// @param modified New modification time (CX time format)
/// @param accessed New access time (CX time format)
...
/// @return true if all directories created or exist, false on error
...
/// @return true if successful, false on error (not empty, doesn't exist, etc.)
...
/// @param to Destination file path (will be created or overwritten)
/// @return true if successful, false on error
...
///
/// @param out Receives the OS filesystem path
/// @param vfs VFS instance
/// @param path VFS path to resolve
/// @return true if path is backed by VFSFS and OS path retrieved, false if path uses a different
/// provider type
...
///
/// @param iter Active search iterator
/// @return true if another entry found, false if no more entries
...
/// multiple times.
///
/// @param iter Search iterator to finish
...
/// @return true if iterator has a valid current entry, false otherwise
_meta_inline bool vfsSearchValid(_In_ FSSearchIter* iter)
{
    return iter->_search;
}
...
///
/// @param vfs VFS instance
/// @param path File path to open (can be relative or absolute)
/// @param flags Combination of FS_Read, FS_Write, FS_Create, etc.
/// @return VFSFile handle on success, NULL on failure
///
...
enum VFSFlags {
    ...
    VFS_ReadOnly = 0x00000001,   ///< Disallow write operations on this VFS/layer. Read-only
                                 ///< providers can serve as base layers with writable overlays on
                                 ///< top

    VFS_CaseSensitive = 0x00000002,   ///< Case-sensitive path matching. Efficiency of this flag (as
                                      ///< well as of the default case-insensitive mode) may vary
    ...
    VFS_NewFiles = 0x00000008,   ///< Direct new file creation to this layer. When multiple writable
                                 ///< layers exist, this determines where new files are created

    VFS_AlwaysCOW = 0x00000010,   ///< Force Copy-On-Write for all writes. Writing to files from
                                  ///< lower layers creates a copy in this layer, even if lower
                                  ///< layer is writable. Useful for mod overlays.
