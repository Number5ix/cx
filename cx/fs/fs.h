/// @file fs.h
/// @brief Platform-specific filesystem operations

/// @defgroup fs_fs Native Filesystem
/// @ingroup fs
/// @{
///
...
///
/// @param out String to receive the converted path (prior contents destroyed)
/// @param platformpath Platform-specific path to convert
...
///
/// @param out String to receive the platform path (prior contents destroyed)
/// @param path CX normalized path to convert
...
///
/// @param out String to receive the current directory (prior contents destroyed)
...
///
/// @param cur Path to set as current directory (can be relative or absolute)
/// @return true if successful, false on error (directory doesn't exist, no permission, etc.)
...
///
/// @param out String to receive the absolute path (prior contents destroyed)
/// @param path Relative or absolute path to convert
/// @return true if the path was made absolute (was relative), false if already absolute
...
///
/// @param out String to receive the executable path (prior contents destroyed)
...
///
/// @param out String to receive the executable directory (prior contents destroyed)
...
///
/// @param path Path to query (can be relative or absolute)
/// @param stat Optional pointer to receive detailed metadata (can be NULL)
/// @return FS_Nonexistent if the path doesn't exist, FS_Directory if the path is a directory,
...
/// @return true if the path exists (as any type), false otherwise
...
///
/// @param path Path to the file or directory to modify
/// @param modified New modification time
/// @param accessed New access time
/// @return true if successful, false on error (file doesn't exist, no permission, etc.)
...
/// fsCreateAll() instead.
///
/// @param path Path where the new directory should be created
/// @return true if successful or directory already exists, false on error (parent doesn't exist, no
/// permission, path is a file, etc.)
...
/// @return true if all directories were created or already existed, false on error (no permission,
/// path component is a file, etc.)
...
/// @return true if successful, false on error (doesn't exist, not empty, no permission, etc.)
...
/// @return true if successful, false on error (doesn't exist, is a directory, no permission, etc.)
...
/// @return true if successful, false on error (source doesn't exist, no permission, etc.)
...
///
/// @param iter Iterator structure to initialize (will be cleared first)
...
///
/// @param iter Active search iterator (from fsSearchInit)
/// @return true if another entry was found (iter->name is valid), false if no more entries (end of
/// directory reached)
...
///
/// @param iter Search iterator to clean up
...
///
/// @param iter Search iterator to check
/// @return true if iter->name contains a valid entry, false otherwise
///
/// Example:
/// @code
///   FSSearchIter iter;
