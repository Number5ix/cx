...
STRNUM_IMPL(int32, int32, uint32, strToInt32,
            neg ? (uint32) - (MIN_INT32 + MAX_INT32) + MAX_INT32 : MAX_INT32)
STRNUM_IMPL(uint32, int32, uint32, strToUInt32, MAX_UINT32)
STRNUM_IMPL(int64, int64, uint64, strToInt64,
            neg ? (uint64) - (MIN_INT64 + MAX_INT64) + MAX_INT64 : MAX_INT64)
STRNUM_IMPL(uint64, int64, uint64, strToUInt64, MAX_UINT64)

// integer to string -------------------------------------------------------------------

_Use_decl_annotations_
uint8* _Nonnull _strnum_u64toa(uint8 buf[STRNUM_INTBUF], uint32* _Nullable len, uint64 val,
                               uint16 base, uint32 mindigits, char sign, bool upper)
{
    ...
    *--p = 0;   // null terminator
    ...
    if (buflen < mindigits) {
        for (size_t i = 0; i < mindigits - buflen; i++) *--p = '0';
        buflen = mindigits;
    }
...
static _meta_inline bool
_strFromIntHelper(strhandle out, uint64 val, uint16 base, uint32 mindigits, char sign, bool upper)
{
