...
static void _strMkOptimalRoperef(_Inout_ str_roperef* out, _In_ strref s, uint32 off, uint32 len)
{
    ...
    if (_strHdr(s) & STR_ROPE) {
        ...
        if (data->right.str && off >= data->left.len &&
...
static void _strRotateLeft(_In_ strhandle_v top)
{
    ...
    if (cdata->right.str) {
        ...
        string_v newleft = _strCreateRope((strref_v)tdata->left.str,
                                          tdata->left.off,
                                          tdata->left.len,
                                          (strref_v)cdata->left.str,
                                          cdata->left.off + tdata->right.off,
                                          min(cdata->left.len - tdata->right.off, tdata->right.len),
                                          true);

        // now create what will be the new root:
        // the new left node we just created followed by the child's right node
        newtop = _strCreateRope(newleft,
                                0,
                                0,
                                cdata->right.str,
                                cdata->right.off,
        ...
        // child only has a left tree somehow, this is a degenerate tree so just move the left node
        // up and delete the child
        devAssert(tdata->left.str && cdata->left.str);
        newtop = _strCreateRope((strref_v)tdata->left.str,
                                tdata->left.off,
                                tdata->left.len,
                                (strref_v)cdata->left.str,
                                cdata->left.off + tdata->right.off,
...
static void _strRotateRight(_In_ strhandle_v top)
{
    ...
    if (cdata->right.str) {
        ...
        newright = _strCreateRope(cdata->right.str,
                                  cdata->right.off,
        ...
        newtop = _strCreateRope((strref_v)cdata->left.str,
                                cdata->left.off + tdata->left.off,
...
string_v _strCreateRope(strref_v left, uint32 left_off, uint32 left_len, strref right,
                        uint32 right_off, uint32 right_len, bool balance)
{
    ...
    left_len = left_len ? min(left_len, _strFastLen(left) - left_off) :
                          _strFastLen(left) - left_off;
    if (right) {
        right_len = right_len ? min(right_len, _strFastLen(right) - right_off) :
                                _strFastLen(right) - right_off;
...
string_v _strCreateRope1(strref_v s, uint32 off, uint32 len)
{
    ...
    if (off + len <= sdata->left.len || off >= sdata->left.len || !sdata->right.str) {
        ...
        _strMkOptimalRoperef(&data->left,
                             sdata->left.str,
                             sdata->left.off + off,
                             sdata->left.len - off);
        _strMkOptimalRoperef(&data->right,
                             sdata->right.str,
                             sdata->right.off,
...
uint32 _strRopeFastCopy(strref_v s, uint32 off, uint8* _Nonnull buf, uint32 bytes)
{
    ...
    if (off < data->left.len)
        leftcopy = _strFastCopy(data->left.str,
                                off + data->left.off,
                                buf,
                                min(bytes, data->left.len - off));
    if (bytes == leftcopy)
        return leftcopy;   // all done from left side only!

    rightcopy = _strFastCopy(data->right.str,
                             off + data->right.off + leftcopy - data->left.len,
                             buf + leftcopy,
                             bytes - leftcopy);
...
_strRopeRealStrPart(_Inout_ str_roperef* _Nonnull ref, uint32 off, _Out_ string* _Nonnull rs,
...
_Success_(return) bool _strRopeRealStr(_Inout_ strhandle_v s, uint32 off, _Out_ string* _Nonnull rs,
                                _Out_ uint32* _Nonnull rsoff, _Out_ uint32* _Nonnull rslen,
                                _Out_ uint32* _Nonnull rsstart, bool writable)
{
    if (!(_strHdr(*s) & STR_ROPE))
    ...
    if (_strRopeRealStrPart(&data->left, off, rs, rsoff, rslen, rsstart, writable))
        return true;
    if (_strRopeRealStrPart(&data->right,
                            off - data->left.len,
                            rs,
                            rsoff,
                            rslen,
